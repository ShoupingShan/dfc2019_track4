...
parser.add_argument('--data_dir', default=os.path.join(ROOT_DIR, 'data', 'dfc', 'train'),
...
parser.add_argument('--decay_step', type=int, default=68720,
                    help='Decay step for lr decay [default: 68720 (=40x number of training point clouds after split)]')
...
parser.add_argument('--fold', type=int, default=1, help='Fold validation location [default: 1]')
...
os.system('cp %s %s' % (os.path.join(FLAGS.data_dir, 'dfc_train_metadata.pickle'),
                        LOG_DIR))  # copy of training metadata
LOG_FOUT = open(os.path.join(LOG_DIR, 'train.log'), 'a')
...
TRAIN_DATASET = dfc_dataset.DFCDataset(root=FLAGS.data_dir, npoints=NUM_POINT, split='train',
                                       log_weighting=FLAGS.log_weighting, extra_features=FLAGS.extra_dims)
TEST_DATASET = dfc_dataset.DFCDataset(root=FLAGS.data_dir, npoints=NUM_POINT, split='val',
            ...
            for i in range(NUM_GPUS):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device('/gpu:%d' % (i)), tf.name_scope('tower_gpu_%d' % (i)) as scope:
                        ...
                        # input()
                        pred, end_points = MODEL.get_model(pc_batch, is_training=is_training_pl, num_class=NUM_CLASSES,
                                                           bn_decay=bn_decay)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for l in range(NUM_CLASSES-1):
        per_class_str += '  \nclass %d[%d] acc: %f, iou: %f;' % (TEST_DATASET.decompress_label_map[l+1],l+1,total_correct_class[l+1]/float(total_seen_class[l+1]),iou[l])
